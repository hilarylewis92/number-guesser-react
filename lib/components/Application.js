...
export default class Application extends Component {
  ...
  outsideRangeMessage (){
    const { userGuess, defaultMinimum, defaultMaximum } = this.state
    if( userGuess < defaultMinimum || userGuess > defaultMaximum ){
      this.setState({
        outsideRangeMessage: 'Guess a number inside your set range.',
        messageHint: ''
      });
    }else{
      this.setState({
  ...
  messageHint(){
    ...
    if(userGuess < randomNumberGenerated){
      ...
      this.setState({
        ...
        setRangeMessage: 'Your new range has been set.'
      });
    }
  }
